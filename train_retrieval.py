...
def evaluate(model, epoch, args):
    ...
    for doc in tqdm(global_passages):
        passage_inputs = tokenizer(doc['text'], padding=True,
    ...
    for question, passage_id in tqdm(zip(dev_dataset.questions, dev_dataset.context_ids)):
        question_inputs = tokenizer(question, padding=True,
...
if __name__ == "__main__":
    ...
    ctx_model = BertModel.from_pretrained(model_name)

    model = RetrievalModel(question_model, ctx_model)
    optimizer = AdamW(model.parameters(), lr=1e-5)

    train_dataset = RetrievalDataset("data/train_retrieval.json", args.training_size)
